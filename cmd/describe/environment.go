	...
	for _, svc := range env.Services {
		if serviceName == "" {
			...
			if serviceName == *svc.Name {
				...
				var formattedComponentData = string(componentBytes)
				formattedComponentData, _ = util.ConvertJSONToYAML(formattedComponentData)
				lines := strings.Split(formattedComponentData, "\n")

				for i, line := range lines {
					lines[i] = "\t" + line
				}
				formattedComponentData = strings.Join(lines, "\n")
				// Add two tabs before each line in the string
				customServicesOp = append(customServicesOp, formattedComponentData)
